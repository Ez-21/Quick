    ...
    return (
        <div>
            {/* 提示词弹窗 */}
            {
                callShow && (
                    <div className={style.callWord}>
                        <div className={style.callWordTitle}>
                            选择提示词
                        </div>
                        ...
                        <div className={style.btnBoxBottom}>
                            <div className={style.cancle} onClick={() => setCallShow(false)}>取消</div>
                            <div className={style.save} onClick={() => setCallShow(false)}>保存</div>
                        </div>
                    </div>
                )
            }
            <div className={style.title}>
                ...
                <img src={ModalAdd} alt="" onClick={() => setCallShow(() => !callShow)} />
            </div>
            <div>
                <div className={style.changebox}>
                    <div className={style.btnBox}>
                        {
                            tabs.map(item => {
                                return (
                                    <div key={item.id} style={tabIndex === item.key ? {
                                        background: "rgba(255, 255, 255, 0.2)",
                                        color: "white",
                                    } : undefined} className={style.btnItem} onClick={() => changeTab(item.key)}>
                                        {item.label}
                                    </div>
                                )
                            })
            ...
            <div className={style.modalBox}>
                ...
                <div className={style.select}>
                    <Select options={modal} value={modalVal}>
                ...
                <div className={style.viewChange}>
                    {
                        [1, 2, 3, 4].map((item) => {
                            return (
                                <div onClick={() => viewChange(item)} key={item} style={(item === viewIndex) ? {
                                    color: 'white',
                                    background: 'rgba(255, 255, 255, 0.2)'
                                } : {}}>
                                    {item}
